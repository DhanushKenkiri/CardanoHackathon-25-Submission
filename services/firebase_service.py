...
class FirebaseService:
    ...
    # ============================================
    # AGENT EARNINGS OPERATIONS
    # ============================================
    
    def get_agent_earnings(self) -> Dict[str, int]:
        """Get total earnings for all agents"""
        try:
            ref = db.reference('agent_earnings')
            earnings = ref.get()
            
            if not earnings:
                logger.info("No agent earnings data found")
                return {}
            ...
            logger.info(f"Retrieved earnings for {len(agent_totals)} agents")
            return agent_totals
    ...
    def record_agent_earning(self, earnings_data: Dict) -> bool:
        """Record agent earnings"""
        try:
            ref = db.reference('agent_earnings')
            ref.push(earnings_data)
            
            logger.info(f"Recorded earning for {earnings_data.get('agent_name')}")
            return True
            
        except Exception as e:
            logger.error(f"Error recording agent earnings: {e}")
            return False
    ...
    def create_payment(self, payment_data: Dict) -> Optional[str]:
        ...
        try:
            ...
            payment_data['created_at'] = datetime.utcnow().isoformat() + 'Z'
    ...
    def write_vehicle_entry(self, entry_data: Dict) -> bool:
        """Write vehicle entry event to Firebase (triggers listener)"""
        try:
            ref = db.reference('hardware_events/vehicle_entry')
            ref.push(entry_data)
            
            logger.info(f"✅ Vehicle entry written: {entry_data.get('session_id')}")
            return True
            
        except Exception as e:
            logger.error(f"❌ Error writing vehicle entry: {e}")
            return False
    
    def create_orchestration_record(self, session_id: str, data: Dict) -> bool:
        """Create orchestration tracking record"""
        try:
            ref = db.reference(f'orchestrations/{session_id}')
            ref.set(data)
            
            logger.info(f"Created orchestration record: {session_id}")
            return True
            
        except Exception as e:
            logger.error(f"Error creating orchestration record: {e}")
            return False
    
    def get_orchestration_status(self, session_id: str) -> Optional[Dict]:
    ...
    def update_orchestration_status(self, session_id: str, updates: Dict) -> bool:
        """Update orchestration status"""
        try:
            ref = db.reference(f'orchestrations/{session_id}')
            ref.update(updates)
            
            logger.info(f"Updated orchestration {session_id}: {updates.get('status')}")
            return True
