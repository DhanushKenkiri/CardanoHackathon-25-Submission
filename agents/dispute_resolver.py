...
class DisputeResolverAgent:
    ...
    def create_dispute(self, dispute_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            # Validate input
            if not dispute_data:
                raise ValueError("Dispute data is required")
            ...
            firebase_dispute_id = firebase_service.create_dispute(dispute_record)
            if not firebase_dispute_id:
                raise ValueError("Failed to create dispute in Firebase")
